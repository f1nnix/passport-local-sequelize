    ...
    UserSchema.register = function (user, password) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (user instanceof UserSchema.DAO) {
                ...
                self.findByUsername(user.get(options.usernameField), function(err,
        					existingUser) {
        					if (err) {
        						reject(err);
        					} else {
        						if (existingUser) {
        							reject(new Error(util.format(options.userExistsError, user.get(
        								options.usernameField))));
        						} else {
        							resolve(Promise.all([user.setPassword(password), user.setActivationKey()])
        								.then(function() {
        									return user.save();
        								}));
        						}
